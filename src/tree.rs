...
impl Pretree {
    ...
    /// ```
    pub fn store(&mut self, method: String, url_rule: String) {
    ...
    /// ```
    pub fn query(&self, method: &str, url_path: &str) -> Option<&Tree> {
        let t = self.tree_group.get(method).unwrap();
        let (is_exist, node) = t.search(url_path);
        if is_exist {
            Some(node)
        } else {
            None
...
impl Tree {
    ...
    fn insert(&mut self, url_rule: String) {
        ...
        if let Some(current) = current.take() {
            current.rule = url_rule.clone();
            current.is_end = true;
        }
    }

    fn search(&self, url_path: &str) -> (bool, &Tree) {
...
mod tests {
    ...
    fn test_match() {
        ...
        for v in data {
            let method = v[0];
            let source_rule = v[1];
            p.store(method.into(), source_rule.into());
        }
